...
chunkfile<-"data/fineRADstructureOutputs/tansy_pops_fineRADpainter_35_reorder_chunks.out" ## RADpainter output file
mcmcfile<-"data/fineRADstructureOutputs/tansy_pops_fineRADpainter_35_reorder_chunks.mcmc.xml" ## finestructure mcmc file
treefile<-"data/fineRADstructureOutputs/tansy_pops_fineRADpainter_35_reorder_morecombos_chunks.mcmcTree.xml" ## finestructure tree file
### 2) EDIT THIS PATH TO WHERE YOU WANT THE PLOTS:
plotsFolder <- "figures/fineRADstructure/"
### 3) SET VALUES FOR THESE VARIABLES: "analysisName" will be included in output plots
analysisName <- "tansy_pops_fineRADpainter_35_reorder";  maxIndv <- 10000; maxPop<-10000


### 4) EDIT THE PATH TO YOUR COPY of FinestructureLibrary.R
source("scripts/fineRADstructure/FinestructureLibrary.R", chdir = TRUE) # read in the R functions, which also calls the needed packages
...
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=0.8,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=1.2))
dev.off()
...
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],labelsx=labels(popdendclear),labelsatx=labellocs,xcrt=xcrt,cols=some.colorsEnd,ycrt=ycrt,dend=tdend,cex.axis=0.75,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=1.2),hmmar=c(3,0,0,1))
dev.off()
